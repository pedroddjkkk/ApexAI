...
function readBase64AsFile(base64String: string, filename: string, type: string) {
            ...
            onClick={async (e) => {
              ...
              file.onchange = async (e) => {
                if (file.files) {
                  ...
                  fileReader.onload = (e) => {
                    const data = JSON.parse(e.target?.result as string).map((item: any) => {
                      ...
                      const name = item.response.split('name=')[1].split(';')[0];

                      // pega o type do arquivo (type=)
                      const type = item.response.split('type=')[1].split(';')[0];
