...
export default function CompanyConfig({ Data }: Props) {
  ...
  const handleDelete = async (id: string) => {
    ...
    const ret = await axios
      .post(`/api/company-config`, { id: id, action: "delete" })
      .then((e) => {
        if (e.status === 200) {
          // update list
          updateData();
                    ...
                    <AccordionContent className="px-4">
                      <div className="flex justify-between">
                        <div className="flex flex-col text-neutral-500">
                          <span>
                            <span className="text-neutral-950">Criado em:</span>
                          ...
                          <span>
                            <span className="text-neutral-950">CNPJ:</span>{" "}
                            {item.cnpj}
                          </span>
                          <span>
                            <span className="text-neutral-950">
                          ...
                          {item.endereco && (
                            <span>
                              ...
                              {item.endereco[0].rua}, {item.endereco[0].numero},{" "}
                              {item.endereco[0].cidade},{" "}
                              {item.endereco[0].estado}
                            </span>
                          )}
                        </div>
                        <div className="gap-4 flex flex-col lg:flex-row mg:flex-row">
                          ...
                          <Button
                            className="bg-red-500 hover:bg-red-700 text-white font-bold py-2 px-4 rounded-full items-center flex gap-2"
